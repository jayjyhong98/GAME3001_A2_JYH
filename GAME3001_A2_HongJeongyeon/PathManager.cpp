...
void PathManager::GetShortestPath(PathNode* start, PathNode* goal, std::vector<PathConnection*>& EnemyPath)
{
	// std::cout << "Starting pathing..." << std::endl;
	EnemyPath.clear(); // Clear the old saved path so we can save a new one.
	EnemyPath.shrink_to_fit();
	NodeRecord* currentRecord = new NodeRecord(start);
	currentRecord->m_totalCost = start->H(); // Already we see an A* addition.
	s_open.push_back(currentRecord);
	while (s_open.size() > 0)
	...
	else
	{
		...
		{
			//std::cout << "Doing record for: [" << currentRecord->m_node->y/32 << ',' << currentRecord->m_node->x/32 << ']' << std::endl;
			EnemyPath.push_back(currentRecord->m_connection);
			currentRecord = currentRecord->m_fromRecord;
		}
		std::reverse(EnemyPath.begin(), EnemyPath.end());
	}
	// Clean up lists. Uncomment the cout below to see how many records we have to clean up.
	// std::cout << "Cleaning up..." << "open size: " << s_open.size() << " | closed size: " << s_closed.size() << std::endl;
	for (unsigned i = 0; i < s_open.size(); i++)
...
void PathManager::GetEuclidPath(PathNode* start, PathNode* goal)
{
	// std::cout << "Starting pathing..." << std::endl;
	s_euclid_path.clear(); // Clear the old saved path so we can save a new one.
	s_euclid_path.shrink_to_fit();
	NodeRecord* currentRecord = new NodeRecord(start);
	currentRecord->m_totalCost = start->H(); // Already we see an A* addition.
	s_open.push_back(currentRecord);
	while (s_open.size() > 0)
	...
	else
	{
		...
		{
			//std::cout << "Doing record for: [" << currentRecord->m_node->y/32 << ',' << currentRecord->m_node->x/32 << ']' << std::endl;
			s_euclid_path.push_back(currentRecord->m_connection);
			currentRecord = currentRecord->m_fromRecord;
		}
		std::reverse(s_euclid_path.begin(), s_euclid_path.end());
	}
	// Clean up lists. Uncomment the cout below to see how many records we have to clean up.
	// std::cout << "Cleaning up..." << "open size: " << s_open.size() << " | closed size: " << s_closed.size() << std::endl;
	for (unsigned i = 0; i < s_open.size(); i++)
...
void PathManager::GetManhatPath(PathNode* start, PathNode* goal)
{
	// std::cout << "Starting pathing..." << std::endl;
	s_manhat_path.clear(); // Clear the old saved path so we can save a new one.
	s_manhat_path.shrink_to_fit();
	NodeRecord* currentRecord = new NodeRecord(start);
	currentRecord->m_totalCost = start->H(); // Already we see an A* addition.
	s_open.push_back(currentRecord);
	while (s_open.size() > 0)
	...
	else
	{
		...
		{
			//std::cout << "Doing record for: [" << currentRecord->m_node->y/32 << ',' << currentRecord->m_node->x/32 << ']' << std::endl;
			s_manhat_path.push_back(currentRecord->m_connection);
			currentRecord = currentRecord->m_fromRecord;
		}
		std::reverse(s_manhat_path.begin(), s_manhat_path.end());
	}
	// Clean up lists. Uncomment the cout below to see how many records we have to clean up.
	// std::cout << "Cleaning up..." << "open size: " << s_open.size() << " | closed size: " << s_closed.size() << std::endl;
	for (unsigned i = 0; i < s_open.size(); i++)
...
void PathManager::DrawEuclidPath()
{
	for (unsigned i = 0; i < s_euclid_path.size(); i++)
	{
		DEMA::QueueLine({ s_euclid_path[i]->GetFromNode()->x + 16, s_euclid_path[i]->GetFromNode()->y + 16 },
			{ MAMA::HalfwayPoint(s_euclid_path[i]->GetFromNode()->Pt(), s_euclid_path[i]->GetToNode()->Pt()).x + 16,
			MAMA::HalfwayPoint(s_euclid_path[i]->GetFromNode()->Pt(), s_euclid_path[i]->GetToNode()->Pt()).y + 16 }, { 255,255,0,255 });
		DEMA::QueueLine({ MAMA::HalfwayPoint(s_euclid_path[i]->GetFromNode()->Pt(), s_euclid_path[i]->GetToNode()->Pt()).x + 16,
			MAMA::HalfwayPoint(s_euclid_path[i]->GetFromNode()->Pt(), s_euclid_path[i]->GetToNode()->Pt()).y + 16 },
			{ s_euclid_path[i]->GetToNode()->x + 16, s_euclid_path[i]->GetToNode()->y + 16 }, { 255,128,0,255 });
	}
}

void PathManager::DrawManhatPath()
{
	for (unsigned i = 0; i < s_manhat_path.size(); i++)
	{
		DEMA::QueueLine({ s_manhat_path[i]->GetFromNode()->x + 16, s_manhat_path[i]->GetFromNode()->y + 16 },
			{ MAMA::HalfwayPoint(s_manhat_path[i]->GetFromNode()->Pt(), s_manhat_path[i]->GetToNode()->Pt()).x + 16,
			MAMA::HalfwayPoint(s_manhat_path[i]->GetFromNode()->Pt(), s_manhat_path[i]->GetToNode()->Pt()).y + 16 }, { 255,255,0,255 });
		DEMA::QueueLine({ MAMA::HalfwayPoint(s_manhat_path[i]->GetFromNode()->Pt(), s_manhat_path[i]->GetToNode()->Pt()).x + 16,
			MAMA::HalfwayPoint(s_manhat_path[i]->GetFromNode()->Pt(), s_manhat_path[i]->GetToNode()->Pt()).y + 16 },
			{ s_manhat_path[i]->GetToNode()->x + 16, s_manhat_path[i]->GetToNode()->y + 16 }, { 255,128,0,255 });
	}
}
...
void PathManager::ClearEuclid_ManhatPath()
{
	s_euclid_path.clear();
	s_manhat_path.clear();
}
...
std::vector<PathConnection*> PathManager::s_euclid_path;
std::vector<PathConnection*> PathManager::s_manhat_path;